...
const double ATTENUATION_FACTOR = 0.15;
...
// Basic direct lighting with multiple point lights, shadow test, and distance attenuation
Vector3 shadeDirectLighting(const HitInfo& hit,
    ...
    Vector3 ambient(AMBIENT_STRENGTH, AMBIENT_STRENGTH, AMBIENT_STRENGTH);
    Vector3 totalLight(0.0, 0.0, 0.0);

    for (const Light& light : lights) {
        Vector3 lightVector = light.position - hit.point;
        ...
        if (occluded(shadowOrigin, light.position, triangles, hit.triIdx)) {
            continue;
        }

        double diffuseStrength = std::max(0.0, hit.normal.dot(toLight));
        double attenuation = 1.0 / (1.0 + ATTENUATION_FACTOR * distanceSquared);

        Vector3 contribution =
            (mat.albedo * light.colour) *
            (diffuseStrength * attenuation * light.intensity);
...
Vector3 shadePixel(RenderMode mode,
    ...
    if (mode == RenderMode::Flat) {
        ...
        return shadeDirectLighting(hit, triangles, materials, lights);
    }
}
...
// Add a rectangle as two triangles
...
// Build a more scene-like test setup
void buildTestScene(std::vector<Material>& materials,
    ...
    int matFloor = (int)materials.size();
    materials.push_back({Vector3(0.8, 0.8, 0.8), Vector3(0.0, 0.0, 0.0)});

    int matLeftWall = (int)materials.size();
    materials.push_back({Vector3(0.9, 0.3, 0.3), Vector3(0.0, 0.0, 0.0)});

    int matRightWall = (int)materials.size();
    materials.push_back({Vector3(0.3, 0.9, 0.4), Vector3(0.0, 0.0, 0.0)});

    int matBackWall = (int)materials.size();
    materials.push_back({Vector3(0.7, 0.7, 0.9), Vector3(0.0, 0.0, 0.0)});

    int matObject1 = (int)materials.size();
    materials.push_back({Vector3(0.9, 0.8, 0.2), Vector3(0.0, 0.0, 0.0)});

    int matObject2 = (int)materials.size();
    materials.push_back({Vector3(0.2, 0.6, 1.0), Vector3(0.0, 0.0, 0.0)});
    ...
    // Simple scene lights
    Light mainLight;
    mainLight.position = Vector3(1.5, 2.5, 2.0);
    mainLight.colour = Vector3(1.0, 1.0, 1.0);
    mainLight.intensity = 3.0;
    lights.push_back(mainLight);

    Light fillLight;
    fillLight.position = Vector3(-2.0, 1.8, 1.0);
    fillLight.colour = Vector3(0.6, 0.8, 1.0);
...
int main() {
    ...
    // Camera setup
    Camera cam;
    cam.pos = Vector3(0.0, 0.8, 4.5);
    cam.lookAt = Vector3(0.0, 0.2, 0.0);
    ...
    std::vector<Triangle> triangles;
    std::vector<Light> lights;
    buildTestScene(materials, triangles, lights);
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            if (hit.hit) {
                color = shadePixel(mode, hit, triangles, materials, lights);
            }
